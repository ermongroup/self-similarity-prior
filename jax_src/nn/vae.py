...
from src.frax.modules import CollageDecoder, Encoder, Decoder, LatAuxVDCVAEDecoder
...
class VDVAE(nn.Module):
    ...
    def __call__(self, x_in, x_out, rng, return_sample=True):
        ...
        px_z, stats = self.decoder(activations, rng)
        end_time = time() - start_time

        distortion = self.decoder.out_net.negative_log_likelihood(px_z, x_out) 
        ...
        sample = None
        if return_sample:
            sample = self.decoder.out_net.sample(px_z, rng)

        return foward_stats, sample, stats


class VDVAESampler(nn.Module):
    config: dict
    ...
    def __call__(self, n_batch, rng, temperature=None):
        px_z = self.decoder.forward_uncond(n_batch, rng, temperature=temperature)  
        return self.decoder.out_net.sample(px_z, rng)
